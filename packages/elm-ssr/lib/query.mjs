...
const generateElmModule = (namespace, table) => {
  ...
  const schemaName = recordName.charAt(0).toLowerCase() + recordName.slice(1) + "Schema";
  const colNames = columns.map(c => c.elmName + "Col");

  let exports = [recordName, schemaName, ...colNames, "decoder", "all", "insert"];
  if (pk) {
...
--
-- Elmto is canonical: use ${schemaName} + *Col with ElmSsr.Db.Elmto.Query / Repo for rich queries,
...
`;
  ...
  // Local helpers (bool/nullable) used by decoder + insert encoders
  if (hasBool) {
...
decoder =
  ...
  // Elmto Schema
  code += `${schemaName} : Elmto.Schema ${recordName}
${schemaName} =
    Elmto.schema "${tableName}" decoder
`;
  for (const c of columns) {
    const getter = "." + c.elmName;
    const typ = c.elmType === "Int" ? "Elmto.int" : c.elmType === "Float" ? "Elmto.float" : c.elmType === "Bool" ? "Elmto.bool" : "Elmto.string";
    const ctor = c.isNullable ? "optionalField" : "field";
    code += `        |> Elmto.${ctor} "${c.dbName}" ${getter} ${typ}\n`;
  }
  code += `\n`;

  // Column descriptors (Elmto)
  for (const c of columns) {
    code += `${c.elmName}Col : Elmto.Column ${recordName} ${c.elmType}
${c.elmName}Col =
    Elmto.column "${c.dbName}" ${getColEncoder(c.elmType)}

`;
  }

  // Helpers: all (raw for cross-dialect compat; use Repo.all dialect (Query.from ${schemaName}) for Elmto)
